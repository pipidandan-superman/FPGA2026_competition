...
| H12 | G3 M12 部署承接（拆 A1/A2/B 三段） | 2026-09-16 | ◐ A1 全绿；**B0 150MHz 已收敛**；A2 待做 | **A1 五门全绿**：M8 run03（ctrl V1.2 rq_rdy 回归）、M9b run01–03（dma_wr 真写主 + V1.2 非对齐）、M10 run03（阵列 V1.2a Y 真 AXI 写主）、M11 run03（全网七项同数 + head sha 复命中）、CSR/engine run01（首跑过）。**UG479 合规批**（V1–V4 全量：DSP 三级流水 0→3 拍）门链全绿（M11 双跑 ModelSim 同数+sha）。**B0 时序收敛批（2026-09-17，授权 xbuf BMG 输出寄存合同 + 迭代至收敛）**：13 轮迭代 v13 WNS −31.017 → v24 −0.224（requant 桶形消失）→ v25 −0.079 → v26 −0.031 → **v27 +0.005 PASS（fmax 150.11MHz，dsp 140）**；反复病理 = 宽乘法 2-DSP 级联首 DSP 组合穿越（修法：操作数寄存吸收 AREG/BREG / 拆独立寄存积 PREG——requant V1.2g + gemm_array V1.10）；M10 run12/M12 csr run09 与 v26 逐拍同（延迟平移合同）；**M11 v27 run04 双绿**（TB_FULLNET_PASS 七项计数与 v13 逐一相同 + M11_HEADCHK_PASS sha==run04 frame0，归档副本独立复跑亦 MATCH）——**B0 冻结批全链收口**。**A2 loader V2 三件套已授权待做**；B1 整合/bd + M13 板卡需另行授权 | `4_metrics/logs/2026-09-17_yolo7020_ooc_gate_v25/`+`_v26/`+`_v27/`（含详径探针与 WNS 轨迹表）、`..._m5_requant_run04/`、`..._m5_m7_unit_v25/`、`..._m10_gemm_array_run10..12/`、`..._m12_csr_engine_run07..09/`；`proj/ooc_gate/ooc_v13..17,v19,v20..27.tcl`（dcp 不入库）；`7_logs/2026-09-17/01..03` |
| H13 | G3 M13 整合构建 + 板测 | — | ⬜ 未启动 | 与相机管线同位流，板上 vs PC；**需用户单独授权** | 基线 §5 |

**硬件当前边界**：① RTL 仿真证据链全部绿（`2_fpga/3_yolo_zynq/rtl/` 15 文件 + `sim/` 15 TB + 16 生成器/检查器；最新 = gemm_array V1.10/requant V1.2g/addrgen V1.3/ctrl V1.8/xbuf V2.2/dma V1.3/dma_wr V1.5）；② **B0 OOC 150MHz 已收敛（v27 wns +0.005，非工程批处理纯约束）**，但主工程/bd 整合与位流未动——帧率对外承诺仍待 B1/板测；③ 板卡加载/位流替换逐次单独授权；④ `0_diaplay_test` 冻结基线只读。
