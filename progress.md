...
| H12 | G3 M12 部署承接（拆 A1/A2/B 三段） | 2026-09-16 | ◐ A1 全绿；B0 执行/时序未过 | **A1 五门全绿**：M8 run03（ctrl V1.2 rq_rdy 回归）、M9b run01–03（dma_wr 真写主 + V1.2 非对齐）、M10 run03（阵列 V1.2a Y 真 AXI 写主，六项与 run01 同数）、M11 run03（全网七项与 run02 同数 + head sha 复命中）、CSR/engine run01（AXI-Lite CSR + engine 组装，首跑过）；§5 全链重跑义务已履行。**B0 OOC 三跑**：run01 DSP 238/220 超映射（推断式拆双乘法器）→ pe_pack V1.1 DSP48E1 直例 + xbuf V2.0 BMG IP → run02 dsp=142 但 150MHz FAIL（WNS −31.140）→ **UG479 严格合规批**（用户令"严格按手册设计"，全量 V1–V4：C/D 端接 Table 2-2 note 1、控制属性显式化、AREG/BREG/MREG/PREG=1 三级流水，延迟合同 0→3 拍 + ctrl S_DRAIN + acc_en 重定时）门链 **M1/M8/M4-M10/M11 全绿**（M11 双跑 ModelSim：V1.3 批 TB_FULLNET_PASS 3553900 同数 + M11_HEADCHK_PASS sha==run04；V4 前基线同日复验同数）→ run03 dsp=142 稳定无恶化但 **WNS −31.017 未收敛——"0 级流水=根因"被推翻，真属主 = addrgen 描述符捕获 16 位组合除法/取模（95 级）**；dbg7c 分层：requant −24.6 / acc −15.2（高扇出 CE）/ ctrl −11.6 / xbuf −10.8 / PE −8.1。**时序修复（addrgen 除法/requant 分级/CE 复制）= 新范围待授权**。**A2 loader V2 三件套已授权待做**。xsim M11 被 TB 级 X 问题阻塞（PS t_add/t_rscl 在 xsim 写 X，XDBG3 判决 RTL 无罪），M11 以 ModelSim 为准 | `4_metrics/logs/2026-09-16_yolo7020_m{8,9b,10,11,12}_*_run0{3,3,3,3,1}/`（6 目录）+ `..._ug479_compliance/` + `..._m11_fullnet_xsim_failchain/` + `proj/ooc_gate/ooc_{run02,v13}.*` |
