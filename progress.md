...
更新：2026-09-18（A2/B0 收口 + M11 run05 双绿承证 + M12 OOC 60MHz first-light + 板级构建 PASS + M13 两跑双冻结 + JTAG 裸机路线切换 + run9 BD 修正批后同步）。本文件是全项目唯一的顶层进度管理视图，按"软件 / 硬件"两部分维护；每个条目只写状态与结论，详细过程、命令行与原始证据见对应链接。更新纪律：每批次收口时同步更新本文件 + `README.md`（顶层动态）+ `HANDOFF.md`（交接）+ `7_logs/YYYY-MM-DD/`（当日四件套）；任何"PASS"结论以 `4_metrics/logs/` 下的运行证据为准。

**一句话现状**：软件线量化合同与 PS 运行时已板上位级闭环；硬件线 RTL 证据链全绿（M0–M11 + A2 loader V2 收口，M11 run05 全网双绿承证）、OOC 60MHz first-light、板级位流构建 PASS；M13 板测两跑双冻结（/dev/mem 直写 DDR 假设否定）后转 **JTAG 裸机路线**（Vitis 平台/应用工程已建，BD 修正批 run9c 收口中）。
...
| H13 | G3 M13 整合构建 + 板测 | 2026-09-17/18 | ◐ 构建 PASS；板测两跑双冻结→转 JTAG 裸机 | **板级构建 run8 PASS（wns+0.954，yolo_a2.bit/xsa 解出）**：BD yolo_sys（ps7 EES-331 preset + engine module_ref + GP0→CSR 0x43C1_0000/64K + HP0(W读Y写)/HP1(X读) 各 0x0[1G]）。**板端驱动 PC 自测双绿**（pl_m11.py /dev/mem+prog 解释器：PL_M11_PASS convs=63 psops=65 compared=3553900 nerr=0 head_sha256==冻结）。**M13 两跑双冻结**（run01 0x30000000 / run02 0x08000000 证据驱动全空窗口）：下载+身份读两绿→执行期板硬失联（ping/串口死、无 panic）——**DDR 段占用假设否定**；修正根因排序：①/dev/mem RAM 别名 mmap 13MB 直写机制（run01 冻点在纯 CPU 写阶段，引擎未启动）②HP 互连挂死 ③FCLK0 未证。**用户决策转 JTAG 裸机路线**（Vitis 2025.2 平台 yolo_zynq + app_component 已建，JTAG 挂死现场可 xsct 读 PC）。**run9 BD 修正批收口（run9c BOARD_BITSTREAM_PASS wns+0.623，CRITICAL WARNING 总数=0：IOSTANDARD 100→0、18-1056→0、41-967 亦归零——"module_ref 不可根除"旧结论作废，真因=FREQ_HZ 缺失半配置接口）**；用户 GUI 修 DDR/FIXED_IO 接出+IRQ_F2P 勾选，批处理三连 re-apply FREQ_HZ + write_bd_tcl 收编正典 yolo_sys_bd.tcl（IRQ_F2P 物化机制=一次性 set_property -dict 灌入）；yolo_a2.xsa adc0a7d1/bit d1f08549 | `4_metrics/logs/2026-09-1{7,8}_yolo7020_{m13_driver_selftest_run01,board_build_run01,m13_board_run01,board_build_run09}/`、`7_logs/2026-09-17/11` |

**硬件当前边界**：① RTL 仿真证据链全部绿（`2_fpga/3_yolo_zynq/rtl/` 15 文件 + `sim/` 15 TB + 16 生成器/检查器；最新 = gemm_array V2.0c/xrowgen V1.3/requant V1.2g/addrgen V1.3/ctrl V1.8/xbuf V3.0(BMG)/wbuf V3.0/dma V1.4/dma_wr V1.5）；② OOC 150MHz 已收敛（v27 +0.005）；**部署档 v28@150 FAIL(−8.750) → v28b60@60MHz first-light PASS(+0.806)**，板级 BD 构建亦 60MHz PASS——帧率对外承诺待板后优化决策（候选：A2b X2/oc-pair、xrowgen 流水化、提频）；③ M13 Linux+/dev/mem 路线两跑双冻结证死（占用无关），转 JTAG 裸机（Vitis 工程已建，P0/P1/P2 三相规划就绪待用户指令）；④ 板卡加载/位流替换逐次单独授权；⑤ `0_diaplay_test` 冻结基线只读。
