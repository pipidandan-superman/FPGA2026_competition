...
更新：2026-09-15（G3 M11 全网门通过后收口）。本文件是全项目唯一的顶层进度管理视图，按"软件 / 硬件"两部分维护；每个条目只写状态与结论，详细过程、命令行与原始证据见对应链接。更新纪律：每批次收口时同步更新本文件 + `README.md`（顶层动态）+ `HANDOFF.md`（交接）+ `7_logs/YYYY-MM-DD/`（当日四件套）；任何"PASS"结论以 `4_metrics/logs/` 下的运行证据为准。

**一句话现状**：软件线量化合同与 PS 运行时已板上位级闭环；硬件线 G3 卷积引擎 M0–M11 全绿（仿真全网逐位通过），M12 OOC 综合 / M13 板卡集成未启动（需单独授权）。
...
| H12 | G3 M12 OOC 综合（PROD-16×16） | — | ⬜ 未启动 | 150MHz 必过 / 200MHz 目标；Y 真 AXI 写主、X DDR 流式、PS 微操作硬件化三项架构决策随批入场；**需用户单独授权** | 基线 §5/§8 |
| H13 | G3 M13 整合构建 + 板测 | — | ⬜ 未启动 | 与相机管线同位流，板上 vs PC；**需用户单独授权** | 基线 §5 |

**硬件当前边界**：① RTL 仿真证据链全部绿（`2_fpga/3_yolo_zynq/rtl/` 12 文件 + `sim/` 13 TB + 15 生成器/检查器）；② Vivado/OOC 从未运行，**帧率不对外承诺**（BASE-8×16 ≈12.6fps 峰值估算、PROD-16×16 30fps 仅目标）；③ 板卡加载/位流替换逐次单独授权；④ `0_diaplay_test` 冻结基线只读。
