...
顺序：确认v6 ZIP删除且dataset/完整（1893+1893）→ 三轮检索+GitHub/HF页面核证（类别表、许可、文件树API）→ E盘空间检查（105G可用）→ 直连超时确诊后切换hf-mirror镜像后台下载13.4GB单ZIP → 下载完成后核验LFS SHA-256 `461ad574…` → 安全解压抽查 → 确认ZIP内是否含标注JSON（无则另下官方annotations）→ 建4_metrics run归档证据 → README/调研文档入库提交。方向类候选需用户登录，不代为操作；swipe类不得改名为Left/Right。
