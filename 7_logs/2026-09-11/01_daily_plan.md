...
- 将原始 PYNQ 基础镜像和 222654 板测成功集成镜像统一归档到 `9_pynq/sd`。
- 修订 README、教程和 HANDOFF，使所有复现入口指向同一成功镜像和哈希。
- 完整 IMG 不进入 Git，仅提交说明、清单和必要的小型启动 ZIP。
- 不修改 `2_fpga` 冻结工程；历史 IMG 删除仅限明确失败、重复和可再生中间文件。

## EES-331 工作区基线收敛

- 以 EES-331 为唯一硬件基线，同时保留 Vitis/JTAG 与 PYNQ/SD 已验证结果。
- 清理旧 IMG、旧 Builder、阶段性工程、Vivado/Vitis 缓存和顶层误生成目录。
- 固化 Vitis 发布文件、PYNQ 镜像清单和复现入口，并完成全面路径审计。
...
用户授权为现有 GUI 增加自定义输出路径。发布 v0.2.2，支持浏览、输入及留空默认；原始构建证据固定保留于 `4_metrics/logs`。验证中文空格路径、重复构建不覆盖、无效目录与复制读回失败。证据目录：`4_metrics/logs/2026-09-11_sd_builder_output_dir_run01`。
