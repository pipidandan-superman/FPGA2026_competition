...
1. MinerU解析参考PDF；证据 `4_metrics/logs/2026-09-13_pc_ps_pl_diagrams_mineru_run01`。
2. 核对固定发布HWH、视频链路、ACTL寄存器、UART/LED及PC/PS控制流程，保存源哈希与模块接口摘录。
3. 生成原生可编辑图及PDF/PNG预览；详细流程覆盖启动、正常事务、失败和恢复。
4. 校核连线方向、地址、时钟/复位、状态边界及排版，更新文档索引和日志。制图证据 `4_metrics/logs/2026-09-13_pc_ps_pl_diagrams_run01`。
...
## 个人分支关键资产发布与 main 合并步骤

1. 在独立个人分支工作树中以 `origin/main` 为基准核对已入分支和工作区关键资产。
