#!/usr/bin/env python3
"""pl_freq100_run25.py - frequency gate: same BD as run24, FCLK0 50 -> 100 MHz.

The ONLY variable this run is the fabric clock (PS7 FCLK0 divisor chain
IO_PLL(1000MHz on this board) / div0=5 / div1=2 = 100 MHz, per HWH).
Expectation discipline: GEMM arithmetic and DMA data are cycle-count-semantic,
not frequency-dependent, so with seeds identical to run23/run24 every gate
value below must be BIT-IDENTICAL (y_count 128/32/128, stale 0x25,
STATUS 0x28/0x38, DMASR IOC+Idle 0x1002 patterns) - any deviation is a
frequency-coupled defect, not noise.

New gate CK_F (before any functional test):
  CK_Fa fclk0 pre-download (boot state) via pynq Clocks (model-derived,
        logged only - pynq 3.0.1 assumes a 50 MHz ref clock, this board
        has a 33.3333 MHz crystal, so every pynq MHz report is 1.5x high)
  CK_F0 layout adjudication (attempt-2 lesson + probe_clk2.py): pynq
        3.0.1 ps.py ZYNQ_CLK_FIELDS IS the hardware layout:
        FPGA0_CLK_CTRL 0xF8000170 = DIVISOR0[13:8] / DIVISOR1[25:20] /
        SRCSEL[5:4]; bits [3:0] are reserved and write-ignored (proved on
        board: a 0x...0205 write read back 0x...0200). So
        Overlay.download() already programs the HWH intent (div0=5,
        div1=2) correctly -> 0x00200500 = IO_PLL (FBDIV 30 x 33.3333 =
        1000 MHz true) / 5 / 2 = 100.000 MHz true. The ONLY pynq defect
        is the reference-clock model (50 vs 33.3333 -> all MHz reports
        1.5x high); its fclk0_mhz getter may raise ZeroDivisionError -
        always logged, never gated on.
  CK_Fb gate (verify-only, NO SLCR writes): raw 0xF8000170 == 0x00200500
        AND true MHz from live IO_PLL_CTRL FBDIV [18:12] (ref 33.3333
        MHz) in [99,101].
All other gates/sequences verbatim from run24 pl_b2_loopback.py:
  CK1 HWH contract / CK2-4 download+liveness+fresh zocl lock / CK5 fork
  sacrificial dual first reads + triple identity
  Path A: CK6 S1 / CK7 S2 / CK8 S3 / CK9 soft_rst (run23 verbatim)
  Path B: DB1-DB7 DMA loopback discipline (register model from XCI)
Verdict: BOARD_FREQ100_PASS only if every gate passed.
...
def main():
    ...
    if not results['CK1']:
        emit('CKz BOARD_FREQ100_FAIL hwh_contract'); return 1
    ...
    pre = lock_count()
    fclk_pre = Clocks.fclk0_mhz
    emit('CK_Fa fclk0_pre=%s MHz (boot state, before download)' % fclk_pre)
    ...
    if not results['CK4']:
        emit('CKz BOARD_FREQ100_FAIL load'); return 1

    # ---- CK_F: FCLK0 frequency gate (the ONLY variable this run) ----
    # Verify-only, NO SLCR writes: download() itself programs the HWH intent
    # (div0=5, div1=2) through pynq's ZYNQ_CLK_FIELDS, which attempt-2
    # proved IS the hardware layout (bits [3:0] reserved write-ignored).
    # Expected raw = 0x00200500 = IO_PLL/5/2 = 100.000 MHz true.
    slcr = MMIO(0xF8000000, 0x200)
    raw170 = slcr.read(0x170)                    # SLCR FPGA0_CLK_CTRL live
    io_fb = (slcr.read(0x108) >> 12) & 0x7F      # IO_PLL_CTRL FBDIV [18:12]
    io_mhz = io_fb * (1000.0 / 30.0)             # crystal 33.3333 MHz (probe)
    f_div0, f_div1 = (raw170 >> 8) & 0x3F, (raw170 >> 20) & 0x3F
    f_src = (raw170 >> 4) & 0x3
    f_true = io_mhz / (f_div0 * f_div1) if (f_src == 0 and f_div0 and f_div1) else -1.0
    pynq_post = 'n/a'
    try:
        pynq_post = Clocks.fclk0_mhz             # model-derived, 1.5x high;
    except Exception as exc:                     # kept for the record only
        pynq_post = 'ERR %r' % exc
    results['CK_F'] = (raw170 == 0x00200500) and (99.0 <= f_true <= 101.0)
    emit('CK_F0 raw FPGA0_CLK_CTRL=%08X (exp 0x00200500; download programs '
         'it, no repair write this attempt)' % raw170)
    emit('CK_Fb io_pll FBDIV=%d -> %.2f MHz src=%d div0=%d div1=%d true=%.3f MHz '
         'pynq_report=%s -> %s'
         % (io_fb, io_mhz, f_src, f_div0, f_div1, f_true, pynq_post,
            'PASS' if results['CK_F'] else 'FAIL'))
    if not results['CK_F']:
        emit('CKz BOARD_FREQ100_FAIL fclk'); return 1
    ...
    if not msg.startswith('OK'):
        emit('CKz BOARD_FREQ100_FAIL first_read'); return 1
    gid = rd(g, A_ID)
    ...
    if not results['CK5']:
        emit('CKz BOARD_FREQ100_FAIL identity'); return 1
    ...
    if not results['CK6']:
        emit('CKz BOARD_FREQ100_FAIL s1'); return 1
    ...
    if not bank_load(g, Wm2, Xm2, 40, 0, 1):
        emit('CKz BOARD_FREQ100_FAIL s2_load'); return 1
    ...
    if not results['CK7']:
        emit('CKz BOARD_FREQ100_FAIL s2'); return 1
    ...
    if not bank_load(g, Wm3, Xm3, 48, 0, 0):
        emit('CKz BOARD_FREQ100_FAIL s3_load0'); return 1
    wr(g, A_GEOM, 48)
    wr(g, A_JOBCFG, 0x3)
    wr(g, A_CTRL, 1)
    t0 = time.time()
    ...
    if not bank_load(g, Wm3, Xm3, 48, 48, 1):
        emit('CKz BOARD_FREQ100_FAIL s3_load1'); return 1
    ...
    if not results['CK8']:
        emit('CKz BOARD_FREQ100_FAIL s3'); return 1
    ...
    if not results['CK9']:
        emit('CKz BOARD_FREQ100_FAIL sft_rst'); return 1
    ...
    if not results['DB1']:
        emit('CKz BOARD_FREQ100_FAIL dma_reset_state'); return 1
    ...
    if not results['DB2']:
        emit('CKz BOARD_FREQ100_FAIL dma_bringup'); return 1
    ...
    if not results['DB3']:
        emit('CKz BOARD_FREQ100_FAIL dma_basic'); return 1
    ...
    if not results['DB4']:
        emit('CKz BOARD_FREQ100_FAIL dma_sweep'); return 1
    ...
    if not results['DB5']:
        emit('CKz BOARD_FREQ100_FAIL dma_b2b'); return 1
    ...
    if not results['DB6']:
        emit('CKz BOARD_FREQ100_FAIL dma_reset'); return 1
    ...
    if not results['DB7']:
        emit('CKz BOARD_FREQ100_FAIL cross_liveness'); return 1
    ...
    ok = all(results.get(k) for k in
             ('CK1', 'CK_F', 'CK4', 'CK5', 'CK6', 'CK7', 'CK8', 'CK9',
              'DB1', 'DB2', 'DB3', 'DB4', 'DB5', 'DB6', 'DB7'))
    emit('CKz %s' % ('BOARD_FREQ100_PASS' if ok else 'BOARD_FREQ100_FAIL'))
