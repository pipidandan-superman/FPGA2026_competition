 ...
 * (a frame just completed), copies that slot into a double-buffered private
 * snapshot (ping-pong, so the buffer being sent is never overwritten) with
 * the CRC32 fused into the copy loop, then submits it. The first two S2MM
 * completions are skipped: the camera stream may still be unstable before
 * its SCCB configuration settles, which showed as garbage startup frames. */
...
static unsigned char cam_snap[2][FRAME_BYTES]; /* ping-pong snapshot buffers */
static uint32_t cam_snap_idx = 0; /* index of the buffer NOT yet submitted */
static uint32_t cam_last_write = 0xFFFFFFFFU; /* force initial skip */
static uint32_t cam_w_changes = 0; /* counts S2MM completions */

#define CAM_COPY_CHUNK 65536U /* 64 KB per copy slice */
#define CAM_SKIP_INITIAL_FRAMES 2U

static uint32_t crc_table[256];
static uint32_t crc_table_ready = 0;

static uint32_t crc32_chunk(uint32_t state, const unsigned char *data, uint32_t length)
{
    uint32_t index;
    ...
    for (index = 0; index < length; ++index) {
        state = crc_table[(state ^ data[index]) & 0xFFUL] ^ (state >> 8);
    }
    return state;
}

/* Copy one camera slot into dst in 64 KB chunks with the CRC32 fused into
 * the same pass; services the Ethernet stack between chunks so RX never
 * stalls. Returns the whole-frame CRC32. */
static uint32_t copy_camera_snapshot(unsigned char *dst, const unsigned char *src)
{
    ...
    while (copied < FRAME_BYTES) {
        ...
        memcpy(&dst[copied], &src[copied], n);
        crc_state = crc32_chunk(crc_state, &dst[copied], n);
        copied += n;
...
static void eth_service(void)
{
    ...
    if (write_slot != cam_last_write) {
        ...
        cam_w_changes++;
        /* Snapshot only when the sender has drained the previous frame:
         * overwriting a buffer mid-send is what produced the torn frames. */
        if (cam_w_changes > CAM_SKIP_INITIAL_FRAMES &&
            udp_video_tx_pending() == 0U) {
            uint32_t done_slot = (write_slot + FRAME_COUNT - 1U) % FRAME_COUNT;
            unsigned char *dst = cam_snap[cam_snap_idx];
            const unsigned char *src =
                (const unsigned char *)(uintptr_t)(DISPLAY_FB_BASE + done_slot * FRAME_SLOT_BYTES);
            uint32_t crc = copy_camera_snapshot(dst, src);
            udp_video_tx_submit(dst, crc);
            cam_snap_idx ^= 1U;
        }
    }
