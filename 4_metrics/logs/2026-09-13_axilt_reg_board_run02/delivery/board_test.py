...
def main():
    ...
    try:
        ...
        event("HARDWARE_CONTRACT", **validate_reset_contract(args.bit.with_suffix('.hwh')))
        ...
        def load():
            ...
            event("OVERLAY_DOWNLOAD_BEGIN")
            overlay.download()
            event("OVERLAY_LOADED", ip=desc["phys_addr"], clocks=overlay.clock_dict)
            event("IDENTITY_READ_BEGIN")
            driver = Axilt.from_overlay(overlay, args.library)
            event("IDENTITY_PASS", ip_id=driver.read("IP_ID"),
                  abi=driver.read("ABI_VERSION"), caps=driver.read("CAPS"))
            return overlay, driver
