...
RELEASE=Path('E:/competition/4_metrics/logs/2026-09-13_axilt_reset_fix_build_run01/release')
...
remote='/home/xilinx/axilt_test_20260913_run02'
manifest=json.loads((RELEASE/'artifact_manifest.json').read_text())
validate_reset_contract(RELEASE/'AXI_LITE_test.hwh')
paths=[RELEASE/name for name in manifest['files'] if not name.endswith('.xsa')]
paths += [RELEASE/'artifact_manifest.json']
paths += [ROOT/'pynq'/name for name in ('axilt.py','board_test.py','mmio_ordered.c','hardware_contract.py')]
records=[]
(RUN/'delivery').mkdir()
for path in paths:
...
try:
    ...
    (RUN/'upload_result.json').write_text(json.dumps(dict(marker='UPLOAD_HASHES_PASS',remote=remote)))
