...
# ================= PROBE: in-memory state vs exported bytes =================
import hashlib as _hl
sys.path.insert(0, 'E:/competition/2_fpga/3_yolo_zynq/pynq')
from yolo_pkg import YoloPackage as _YP
pkg = _YP(str(RUN))

def _h(a):
    return _hl.sha256(np.ascontiguousarray(a).tobytes()).hexdigest()[:16]

log('--- probe: node state vs package ---')
for nm in ['model.0', 'model.1', 'model.22.cv2.0.2']:
    nd_ = nodes[nm]
    pn = pkg.node(nm)
    log(f'{nm}:')
    log(f'  req  match: {nd_.reqs == [tuple(x) for x in pn["req"]]}  mem[0]={nd_.reqs[0]} pkg[0]={tuple(pn["req"][0])}')
    log(f'  b_q  match: {_h(nd_.b_q)} vs {_h(pkg.b(nm))}  dtype {nd_.b_q.dtype}')
    log(f'  w_q  match: {_h(nd_.w_q)} vs {_h(pkg.w(nm))}')
    log(f'  x_scale mem {nd_.x_scale!r} pkg {pn["in_scale"]!r} eq {nd_.x_scale == pn["in_scale"]}')
    log(f'  y_scale mem {nd_.y_scale!r} pkg {pn["out_scale"]!r} eq {nd_.y_scale == pn["out_scale"]}')
    log(f'  s_stored mem {nd_.s_stored!r} pkg {pn["stored_scale"]!r} eq {nd_.s_stored == pn["stored_scale"]}')
    log(f'  w_scales mem[0] {nd_.w_scales[0]!r} pkg[0] {pn["w_scales"][0]!r}')
    if nd_.lut is not None:
        log(f'  lut  match: {_h(nd_.lut)} vs {_h(pkg.lut(nm))}')

# int_forward on golden[0] canvas, compare model.0..2 vs golden npz
gz = np.load(ROOT / '4_metrics/logs/2026-09-14_yolo7020_g2_quant_intref_run01/golden/golden_00_images89_jpg.npz')
canvas = gz['canvas_u8']
regs, clss, t = int_forward(canvas)
for nm in ['model.0', 'model.1', 'model.2.cv1']:
    same = np.array_equal(t[nm][0], gz['int8_' + nm])
    log(f'int_forward[{nm}] == golden npz: {same} ({int((t[nm][0] != gz["int8_" + nm]).sum())} diff)')

# package-based recompute in-process
from intarith import rne_shift as _rs, sat_i8 as _si
n0 = nodes['model.0']
ndp = pkg.node('model.0')
w_ = pkg.w('model.0'); b_ = pkg.b('model.0').astype(np.int64)
oc_, ic_, kh_, kw_ = w_.shape
x0_ = (canvas.astype(np.int16) - 128).astype(np.int8)[None].transpose(0, 3, 1, 2).copy()
xp_ = np.full((1, ic_, 322, 322), -128, dtype=np.int8)
xp_[:, :, 1:321, 1:321] = x0_
cols_ = np.empty((ic_ * 9, 160 * 160), dtype=np.int32)
pos_ = 0
for c_ in range(ic_):
    for i_ in range(3):
        for j_ in range(3):
            cols_[pos_] = xp_[:, c_, i_:i_ + 320:2, j_:j_ + 320:2].reshape(1, -1)[0]; pos_ += 1
acc_ = (w_.reshape(oc_, -1).astype(np.int32) @ cols_).reshape(1, oc_, 160, 160)
zs_ = (128 * w_.reshape(oc_, -1).sum(axis=1)).astype(np.int64)
out_ = np.empty(acc_.shape, dtype=np.int8)
for c_ in range(oc_):
    M_, s_ = ndp['req'][c_]
    out_[:, c_] = _si(_rs((acc_[:, c_].astype(np.int64) + (b_ + zs_)[c_]) * M_, s_))
lut_ = pkg.lut('model.0')
post_ = lut_[(out_.astype(np.int16) + 128).astype(np.uint8)]
log(f'pkg-recompute model.0 == int_forward t: {np.array_equal(post_, t["model.0"])}')
log(f'pkg-recompute model.0 == golden npz  : {np.array_equal(post_[0], gz["int8_model.0"])}')
d_ = (t["model.0"][0].astype(int) - post_[0].astype(int))
import collections as _cl
log(f'int_forward - pkg-recompute hist: {dict(sorted(_cl.Counter(d_.ravel().tolist()).items()))}')
log('# PROBE DONE')
(RUN / 'probe_console.log').write_text('\n'.join(L) + '\n', encoding='utf-8')
