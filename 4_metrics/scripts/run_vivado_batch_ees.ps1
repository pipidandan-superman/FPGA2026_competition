...
if(-not ('EES.GuiEnvironment' -as [type])) {
...
if($tclText -match '(?i)xsim\.simulate\.log_all_signals\s+\{?1\}?'){
  throw "EES_WAVEFORM_LOGGING_ENABLED $tcl"
}
...
$envPairs=[EES.GuiEnvironment]::Read($GuiPid)
...
foreach($pair in $envPairs){
  $i=$pair.IndexOf('=')
  if($i -gt 0){$psi.Environment[$pair.Substring(0,$i)]=$pair.Substring($i+1)}
}
...
$raw=$out.Result+"`r`n"+$err.Result
$raw | Set-Content -LiteralPath (Join-Path $run 'vivado_console.log') -Encoding utf8
$marker=($raw -match 'EES_VIVADO_RESULT PASS')
$fatal=($raw -match '(?im)^Fatal:|EES_VIVADO_RESULT FAIL|\bERROR:')
...
[pscustomobject]@{state=$state;marker_found=$marker;fatal_or_error=$fatal;exit_code=$proc.ExitCode;timed_out=$timedOut}|ConvertTo-Json | Set-Content -LiteralPath (Join-Path $run 'result.json') -Encoding utf8
