...
def read_xsa(path,assets):
    ...
    if version!='2025.2': raise BuildError(f'第一版仅验证 Vivado 2025.2，当前 XSA 为 {version}。')
    ...
    prefixes=('PCW_FCLK','PCW_FPGA','PCW_ACT_FPGA','PCW_M_AXI_','PCW_S_AXI_',
              'PCW_USE_M_AXI_','PCW_USE_S_AXI_','PCW_IRQ_F2P','PCW_USE_FABRIC_INTERRUPT',
              'PCW_GPIO_EMIO','PCW_EN_EMIO_GPIO')
    dt_changes=[c for c in changes if not c['parameter'].startswith(prefixes)]
    ...
    summary=dict(xsa_path=str(path.resolve()),xsa_sha256=digest(raw),vivado_version=version,
                 ...
                 hwh_name=hwhname,hwh_sha256=digest(hwh),bit_header=bi,
                 ...
                 status='NEEDS_BOARD_DTB' if dt_changes else 'PROFILE_COMPATIBLE')
