...
DEFAULT_BASE='E:/competition/3_host/pynq/download/pynq_z2_v3.0.1.img'
...
class Builder:
    def __init__(self,xsa,vitis=DEFAULT_VITIS,base=DEFAULT_BASE,dtb='',mode='manual',full_image=False,rebuild_fsbl=False,log=print):
        ...
        self.run=WORKSPACE/'4_metrics/logs'/(''+datetime.now().strftime('%Y-%m-%d_sd_builder_%H%M%S_')+uuid.uuid4().hex[:6])
    ...
    def device_tree(self,summary,inputs):
        if summary['device_tree_review_changes'] and not self.custom_dtb:
            changed=', '.join(c['parameter'] for c in summary['device_tree_review_changes'][:10])
            raise BuildError('PS 配置存在需要板级设备树适配的变化：'+changed+'。请提供匹配的新 DTB；工具不会猜测外部器件和驱动。')
        dtb=self.custom_dtb.read_bytes() if self.custom_dtb else (ASSETS/'system.dtb').read_bytes()
        validate_dtb(dtb,inputs['ps'])
        (self.work/'base.dtb').write_bytes(dtb)
        dtc=self.vitis/'bin/dtc.exe'
        self.command([dtc,'-I','dtb','-O','dts','-o','base.dts','base.dtb'])
        mask=sum((1<<i) for i in range(4) if inputs['ps'].get(f'PCW_FPGA_FCLK{i}_ENABLE')=='1')
        override='/include/ "base.dts"\n&{/axi/slcr@f8000000/clkc@100} { fclk-enable = <'+str(mask)+'>; };\n'
        override+='&{/chosen} { stdout-path = "serial0:115200n8"; bootargs = "console=ttyPS0,115200 earlycon root=/dev/mmcblk0p2 rw rootfstype=ext4 rootwait devtmpfs.mount=1 uio_pdrv_genirq.of_id=generic-uio clk_ignore_unused"; };\n'
        (self.work/'board.dts').write_text(override)
...
'''.replace('KERNEL_HASH',hexhash(kernel)).replace('DTB_HASH',hexhash(dtb))
    ...
    def execute(self):
        ...
        try:
            ...
            if self.full_image and not self.base.is_file(): raise BuildError('请提供基础 IMG。')
            ...
            manifest=dict(result='SD_PACKAGE_STATIC_PASS',tool_version='0.1.0',created=datetime.now().isoformat(),
...
def main():
    ...
    try:
        ...
        else: Builder(args.xsa,args.vitis,args.base_img,args.dtb,args.mode,args.full_image,args.rebuild_fsbl).execute()
    except Exception as exc:
