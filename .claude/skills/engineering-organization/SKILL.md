...
description: Safely organize the E:\competition engineering workspace, classify files, normalize live paths, and audit migration references without deleting or overwriting files. Use for explicit file migration or directory organization tasks.
---
...
- Skill archive: `E:\competition\6_skill`.
- Frozen successful FPGA project: `E:\competition\2_fpga`.

不要把新日志写入 `2_log\`、`log\`、`logs\`、`D:\VitA\...` 或旧桌面
competition 路径。不要把仓库根目录当作仿真库、波形或构建临时目录。
...
- 不移动、不删除、不修改 `E:\competition\2_fpga` 中的板级验证基线，除非
  当前用户指令明确授权；即使授权，也先记录源文件哈希和相关证据。
...
- 删除目标均有授权和记录；
- 历史证据未被误删或覆盖；
- 当前工程的日志权威入口是 `E:\competition\7_logs\`；指标原始证据入口是
  `E:\competition\4_metrics\logs\`。若新增项目状态文件，必须在 README 中
  明确其权威性。
