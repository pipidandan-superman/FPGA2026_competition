...
版本：v1.2，2026-09-12。状态：PC与板载蓝牙短时双向通信已实测通过，新版上位机接入与用户手动复现已确认；正式AXI-Lite/BRAM控制、完整B0/B1/B2验收及机械臂适配仍待执行。

### 最新实测状态（优先于下文历史起点）

- BLE Console v1.1 将未配对检查、无缓存服务/名称读取、三轮保持及自动通知集成GUI；
  31项离线/Tk测试、新版后端60.5秒三轮双向、发布EXE按钮双向字节比对通过。
  用户另行确认双向通信成功。详见[新版证据](../../4_metrics/logs/2026-09-12_ble_console_v11_board_run02/REPORT.md)
  和[用户确认](../../4_metrics/logs/2026-09-12_ble_manual_confirmation_run01/REPORT.md)。
  这是基础通路与工具复现完成，不改变下文B0/B1/B2原定出口；后续执行仍须用户明确开始。
