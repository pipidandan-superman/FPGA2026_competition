...
版本：v1.0，2026-09-12。状态：方案已编写；开发、仿真、上板均待逐阶段执行。用户已确认接口架构，本轮授权文档整理和个人分支发布。
...
2026-09-12本机只读枚举显示Intel无线Bluetooth状态OK及Microsoft蓝牙LE枚举器存在；尚未进行BLE扫描或GATT实测。MLT-BT05实际固件、角色能力和UUID未知。厂家BT24标准资料提供FFE0/FFE1/FFE2及双向透传，但LeArm定制固件和具体后缀仍待核实。
...
B阶段不等待完整C阶段设计实现。MLT主机模式不支持或机械臂未到货时，保存B阶段结论并继续C0/C1；不得把该分支写成机械臂直连PASS。各阶段完成后交付结果，按用户后续执行指令推进；本文件不宣称任何新增硬件PASS。
...
下一次执行从G0开始：读取本文件，核验当前基线和PC BLE环境，建立独立UART诊断副本，核实供电极性，再通知用户上电并完成B0/B1。优先顺序不得退回“先把整套寄存器和机械臂全部写完再测蓝牙”。
...
本次发布只承诺方案及接口契约已整理、链接/表格/范围已审查，尚无新增蓝牙、RTL或机械臂实测结论。
