# Reproducible synthesis/implementation/bitstream entry point for the
# integrated display_test + AXI-Lite register + Bluetooth design.
# Usage: vivado -mode batch -source build_axi_ble.tcl -tclargs <new-run-dir>

if {$argc != 1} {
    error "Usage: build_axi_ble.tcl <new-run-dir>"
}
...
write_hw_platform -fixed -include_bit -force \
    -file [file join $release_dir display_test_axi_ble.xsa]
file copy -force $bit_file \
    [file join $release_dir display_test_axi_ble.bit]
file copy -force $hwh_file \
    [file join $release_dir display_test_axi_ble.hwh]
...
puts $output "BLE_READY_EXTERNAL false"
close $output

puts "MAIN_AXI_BLE_BUILD_PASS setup=$setup_slack hold=$hold_slack"
