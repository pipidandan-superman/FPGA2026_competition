"""AXLT ABI 1.0 access for the integrated video overlay."""
import ctypes
import time
...
class OrderedMMIO:
    ...
    def _check(self, offset):
        if (type(offset) is not int or offset % 4 or
                not 0 <= offset <= self.length - 4):
...
class Axilt:
    """Single-command register driver; uncertain commands are never retried."""
    ...
    def from_overlay(cls, overlay, library, ip_name="axi_lite_test_0"):
        ...
        if size != 4096 or int(desc["phys_addr"]) != 0x43C00000:
    ...
    def status(self):
        ...
        return {
            ...
            "done": bool(value & 4),
            "failed": self.failed,
            "accept_seq": self.read("ACCEPT_SEQ"),
            "done_seq": self.read("DONE_SEQ"),
        }
    ...
    def execute(self, value, seq=None, timeout=1.0):
        ...
        observed = {
            "accept_seq": self.read("ACCEPT_SEQ"),
            "done_seq": self.read("DONE_SEQ"),
            "error": self.read("ERROR_CODE"),
            "exec_count": self.read("EXEC_COUNT"),
        }
        if (status & 2 or observed["accept_seq"] != seq or
                observed["done_seq"] != seq or observed["error"] or
                observed["exec_count"] != ((count + 1) & 0xFFFFFFFF) or
                result != (value ^ 0xFFFFFFFF)):
            raise RuntimeError("Hardware result/sequence/count mismatch: " +
                               repr(observed))
        self.io.write(REG["ACTION"], 2)
        if self.read("STATUS") != 1:
            raise RuntimeError("ACK did not restore READY")
        self.failed = False
        return {"seq": seq, "input": value, "result": result, **observed}
