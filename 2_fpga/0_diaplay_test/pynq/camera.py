...
BIT_HASH = '0518c46b4dca4dca83b1e94ae5c1e8cbc8bb68fbf4537e5a90c025a10b76db87'
HWH_HASH = '96eeba65209e66ac3e05a65907f48cc374dab490b9d88e2e72e22689e939a4dd'
stopping = False
...
class Camera:
    def __init__(self, bit):
        ...
        for path, expected in ((bit, BIT_HASH), (bit.with_suffix('.hwh'), HWH_HASH)):
        ...
        log('PL_LOADED', bit_sha256=BIT_HASH, state=state, base=hex(desc['phys_addr']))
        self.mmio = MMIO(desc['phys_addr'], desc['addr_range'])
        # The bare-metal application initializes Ethernet before VDMA. After a
        # Linux boot that implicit sensor/SCCB settling time must be explicit.
        time.sleep(1.0)
        log('SENSOR_SETTLE_COMPLETE', seconds=1.0)
        self.reset()
        ...
        while not seen[0]:
            ...
            if stopping or time.monotonic() > deadline:
                log('FIRST_FRAME_TIMEOUT', seen=seen, mm2s_sr=hex(self.read(4)),
                    s2mm_sr=hex(self.read(0x34)), mm2s_cr=hex(self.read(0)), s2mm_cr=hex(self.read(0x30)))
