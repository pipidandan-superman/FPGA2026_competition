...
def main():
    ...
    try:
        ...
        camera = video.Camera(args.bit, hashes, validate_action_hardware_contract)
        camera.start()
        ...
        while not video.stopping and (not args.seconds or time.monotonic()-started < args.seconds):
            ...
            if now-last_report >= 2:
                ...
                last_report = now
            time.sleep(.001)
        result = dict(state="COMPLETE", frames=frames, packets=packets, changes=changes,
                      seconds=time.monotonic()-started, vdma=camera.status(),
                      action=actions.driver.status(), hdmi="MANUAL_CONFIRMATION_REQUIRED")
