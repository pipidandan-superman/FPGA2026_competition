...
foreach name {axi_lite_slave axi_action_reg_bank action_uart_tx action_command_executor axi_action_control_top} {
    add_files $rtl_source_dir/$name.v
}
add_files E:/competition/2_fpga/0_diaplay_test/rtl/control/video_axi_action_uart_top.v
add_files -fileset constrs_1 E:/competition/2_fpga/2_axi_lite_test/proj/action_v1_pins.xdc
add_files -fileset sim_1 [file join $sim_source_dir tb_axi_action.sv]
...
set_property CONFIG.PCW_FPGA0_PERIPHERAL_FREQMHZ {50.000000} $ps
...
set csr [create_bd_cell -type module -reference video_axi_action_uart_top axi_action_0]
...
connect_bd_net [get_bd_pins ps7/FCLK_CLK0] [get_bd_pins ps7/M_AXI_GP0_ACLK] [get_bd_pins control_interconnect/aclk] [get_bd_pins control_reset/slowest_sync_clk] [get_bd_pins axi_action_0/clk]
...
connect_bd_net [get_bd_pins control_reset/peripheral_aresetn] [get_bd_pins axi_action_0/resetn] [get_bd_pins control_interconnect/aresetn]
connect_bd_intf_net [get_bd_intf_pins ps7/M_AXI_GP0] [get_bd_intf_pins control_interconnect/S00_AXI]
connect_bd_intf_net [get_bd_intf_pins control_interconnect/M00_AXI] [get_bd_intf_pins axi_action_0/S_AXI]
...
foreach pin_name {PL_RS232_TX ACTION_LED} {
    set p [get_bd_pins axi_action_0/$pin_name]
    make_bd_pins_external $p
    set_property name $pin_name [get_bd_ports -of_objects [get_bd_nets -of_objects $p]]
}
validate_bd_design
...
close $out
puts "ACTION_INDEPENDENT_BUILD_PASS"
