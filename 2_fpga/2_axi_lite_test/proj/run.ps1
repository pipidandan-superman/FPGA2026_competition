...
foreach($folder in @('rtl','sim','pynq','doc')){
    Copy-Item -LiteralPath (Join-Path $sourceRoot $folder) -Destination (Join-Path $snapshot $folder) -Recurse
}
New-Item -ItemType Directory -Path (Join-Path $snapshot 'proj') | Out-Null
Get-ChildItem -LiteralPath $PSScriptRoot -File | Where-Object Extension -in @('.tcl','.ps1','.py','.md') |
    Copy-Item -Destination (Join-Path $snapshot 'proj')
$baselinePaths = @(
