...
snapshot = args.run_dir / "source"
snapshot.mkdir()
for name in ("rtl", "sim", "pynq", "doc"):
    shutil.copytree(root / name, snapshot / name)
(snapshot / "proj").mkdir()
for path in (root / "proj").iterdir():
    if path.is_file() and path.suffix in (".tcl", ".ps1", ".py", ".md"):
        shutil.copy2(path, snapshot / "proj" / path.name)
