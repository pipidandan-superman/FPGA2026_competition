...
def main():
    ...
    parser.add_argument("--artifact-stem", default="axilt")
    parser.add_argument("--design-name", default="axilt")
    ...
    tree = ET.parse(release / (stem + ".hwh"))
    reset_contract = validate_reset_contract(release / (stem + ".hwh"))
    ...
    with zipfile.ZipFile(release / (stem + ".xsa")) as archive:
        for ext in (".bit", ".hwh"):
            entry = args.design_name + ext
            if entry not in archive.namelist():
                raise RuntimeError("Expected root design artifact " + entry)
            if archive.read(entry) != (release / (stem + ext)).read_bytes():
    ...
    result = dict(marker="AXILT_RELEASE_AUDIT_PASS", stage="REGISTER_ONLY",
                  design_name=args.design_name, artifact_stem=stem,
                  board_status="PENDING", base=attrs["BASEVALUE"], range=4096,
                  control_clock_mhz=100, reset_contract=reset_contract, files=files)
