...
用户最新指定的实体工程入口为 ../proj/AXI_LITE_test.xpr，BD名 AXI_LITE_test。
用Vivado2025.2打开后可直接Open Block Design。自定义模块以Module Reference形式加入，
实际源码引用独立rtl目录中的 .v 文件；顶层为axi_lite_test_top.v，
自动生成的系统包装为AXI_LITE_test_wrapper.v。

proj/create_local_project.ps1在本地proj首次创建工程，先检查既有仿真PASS及RTL哈希，
然后完成BD、综合、实现、bit和交付配对审查；已有同名工程时拒绝覆盖。
后续通过GUI修改本独立工程时，先归档当前结果，再重新仿真和构建。
下方run.ps1保留run内隔离构建能力，其输出位置与实体工程不同。
...
将同一release目录中的AXI_LITE_test.bit、AXI_LITE_test.hwh、artifact_manifest.json，
以及pynq中的axilt.py、board_test.py、hardware_contract.py、mmio_ordered.c复制到板上独立目录，例如/home/xilinx/axilt_test。
...
sudo env XILINX_XRT=/usr /usr/local/share/pynq-venv/bin/python3 board_test.py --bit ./AXI_LITE_test.bit --manifest ./artifact_manifest.json --run-dir ./evidence/register_run01
```
...
2026-09-13补充：必须检查所有复位输入的极性与常量电平，而非只检查ext_reset_in。本工程aux_reset_in低有效，未使用时必须接1。新增tb_reset_integration.sv使用官方IP仿真网表验证错误接线与正确释放；hardware_contract.py在停止业务和下载之前拒绝错误HWH。旧位流即使此前综合/时序PASS，也禁止继续加载。

现有SD启动PS/Linux后可加载兼容的BIT/HWH；这不是重新运行新XSA的全部PS初始化。DDR/MIO/系统PLL保持兼容，PL时钟从HWH取分频；若改变启动基础配置或内核绑定的PL设备，必须另行审查FSBL/设备树/驱动。用户现授权独立寄存器验收后进行寄存器范围主工程集成，BRAM不作为已完成内容混入。
