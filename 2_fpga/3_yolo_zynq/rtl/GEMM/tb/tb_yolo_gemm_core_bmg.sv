/************************************************************************
 * File Name     : tb_yolo_gemm_core_bmg.sv
 * Developer     : LSL
 * Date          : 2026-09-19
 * Module Name   : tb_yolo_gemm_core_bmg
 * Description   : run15 / IP 重做第五门：yolo_gemm_core V1.1（bank+
 *                 feeder+array 三体封装，bank 为 BMG 真 IP 无参定宽）
 *                 单体门，复跑 run06 全矩阵（G1-G9 tile 清单逐字保留，
 *                 随机流与 run08 一致——数据逐字相同，y 对拍即跨实现
 *                 对照）。相对 run10a：DUT 由"三体分立例化"改为
 *                 core 封装单体（run16 OOC 综合顶层、上板例化单元，
 *                 封装连线一并入测）。
 *
 *   供数机制（相对 run08 直驱）：TB 只做块粒度调度——按块把 Wm/Xm
 *   镜像切片经 64b 流式写口装入 bank 双组（组交替 g=b%2），递交 feeder
 *   作业；feeder↔阵列的块头/宽字流/word_ready 在 core 内部，TB 不再
 *   接触字流。阵列 tile 常量/参数/LUT 口仍由 TB 驱动。
 *
 *   档位：仅 8×16（G2 基线；bank BMG 定宽，4×4/16×16 无通路）。
 *   4×4 单元档等价覆盖已由 run08/run14 冻结（846 checks ×2）。
 *   Kc 为纯逻辑档（装载长度自证），矩阵最深层块 1152。
 *
 ...
 *   - V2.0 (2026-09-19) by LSL : run15 BMG 重做件——DUT 改 yolo_gemm_core
 *     V1.1 单体（bank 真 IP 无参），4×4 档退役，P_KC 删除（Kc 纯逻辑）；
 *     G1-G9 刺激/golden/判据逐字保留。
 ************************************************************************/

`timescale 1ns/1ps

module tb_yolo_gemm_core_bmg #(
    parameter integer P_TO = 8,     //仅 8（bank BMG 定宽）
    parameter integer P_TN = 16     //仅 16（bank BMG 定宽）
);
    ...
    // ---- DUT：core 单体（bank[BMG 真IP]+feeder+array 封装）----
    yolo_gemm_core #(.P_TO(P_TO), .P_TN(P_TN)) u_core (
        .clk_i(clk), .rst_i(rst),
        ...
        .ld_w_grp_i(ld_w_grp), .ld_x_grp_i(ld_x_grp),
    ...
    initial begin        checks = 0; errors = 0;
        ...
        $display("EES_ARR_INFO blocks_fed=%0d blk_done=%0d ld_done=%0d (Kc logical)",
                 blocks_fed, blk_done_seen, ld_done_seen);
        if (errors == 0)
...
// ---- 薄包装：run15 单档（bank BMG 定宽 8×16；4×4 已退役，见文件头）----
module tb_gemm_core_8x16_bmg;
    tb_yolo_gemm_core_bmg #(.P_TO(8), .P_TN(16)) u ();
endmodule
