/************************************************************************
 * File Name       : tb_yolo_gemm_bankfeeder_bmg.sv
 * Developer       : LSL
 * Date            : 2026-09-19
 * Project Name    : AMD embodied sorting / EES-331 XC7Z020
 * Module Name     : tb_yolo_gemm_bankfeeder_bmg
 * Description     : run12 bank+feeder one-body gate -- BMG IP redo
 *                   (user order sequence 2, second gate). Adapted from
 *                   the run09 TB with ONE functional change: the bank
 *                   is the V2.0.1 BMG-IP instance (widths FIXED at the
 *                   G2 8x16 baseline, no width/depth generics). Tier
 *                   coverage shifts accordingly: small-shape control
 *                   is exercised by SMALL LOGICAL Kc at 8x16 (KC=8
 *                   tier) instead of a 4x4 shape; Kc is purely logical
 *                   (proven by load length <= 1024 physical depth).
 *
 ...
 *
 *   Stage matrix (carried over from run09 at identical strength):
 ...
 *   wr_ready sampled only AFTER a posedge (run11 lesson: the value
 *   read in the same delta as driving wr_valid is stale).
 ************************************************************************/
`timescale 1ns/1ps
module tb_yolo_gemm_bankfeeder_bmg #(
    ...
    // bank V2.0.1：BMG IP 存储，宽度固定 G2 基线（无参数）
