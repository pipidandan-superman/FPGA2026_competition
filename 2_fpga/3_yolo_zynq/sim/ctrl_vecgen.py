...
def build():
    ...
    # V1.8（2026-09-17 用户授权）：xbuf BMG 输出寄存合同（读延迟 1→2，
    # xbuf V2.2/gemm_array V1.6）使末 K 拍 tk → acc 落地 tk+5 → requant
    # 采样 tk+6，DRAIN 3→4 拍（drain 0→1→2→3，第四拍末转 RQ）。
    ...
    def step(valid, oc, n, k, last):
        ...
        if s == 'IDLE':
            ...
            # V1.8：4 拍排空（RTL drain_cnt_r 0→1→2→3，=3 的拍末转 RQ）
            drain_cyc[0] += 1
            if r['drain'] >= 3:
    ...
    # V1.3/V1.8：每 tile 恰 4 拍 DRAIN（xbuf V2.2 合同，授权 2026-09-17）
    assert drain_cyc[0] == 4 * tiles_ref, (drain_cyc[0], tiles_ref)
...
def main():
    ...
    manifest = {
        ...
        'contract': 'descriptor stream -> tile loop (n outer, oc inner) '
                    ...
                    'rdy replayed from drv_rdy.hex); V1.8 S_K -> 4-cycle '
                    'S_DRAIN -> S_RQ (xbuf V2.2 BMG output-reg contract: '
                    'operands meet D+2, products D+5, last ren tk -> acc '
                    'tk+5 -> rq sample tk+6; 2026-09-17 用户授权)',
