# ============================================================
# OOC gate v27 -- B0 iteration 13 (2026-09-17)
#   gemm_array V1.10 only: row-address multiply split. v26 top10
#     sole family oc_g_d1_r_reg[13]/C -> row_addr_d2_r0/PCIN
#     -0.031: the V1.5 fused mult+add mapped a 2-DSP cascade with
#     DSP1 traversed combinationally (A -> mult -> adder -> PCOUT
#     4.04ns) -- same pathology requant V1.2g fixed. Split: the
#     16x16 multiply gets its own registered product row_prod_d2_r
#     (single DSP PREG, no cascade in arc), ybase+n_base adds run
#     in fabric in parallel (row_sum_d2_r), combine at d3
#     (row_addr_d3_r). cfg terms snapshotted at d1 -- no cfg read
#     after d1. mod-2^32 associative: bit-exact, d9 presentation
#     unchanged (M10/M12 v27 should be cycle-identical to v26).
#   requant/addrgen/ctrl unchanged (v26 unit gates stand).
#   Previous: v26 FAIL wns -0.031 (fmax 149.30). Target: wns >= 0.
#   对照 run (ooc_v26) 结果见 ooc_v26_drv.log。
# ============================================================
# =====================================================================
...
# batch : vivado -mode batch -source ooc_v27.tcl -nojournal
...
report_utilization    -file $OUT/ooc_v27_utilization.rpt
report_timing_summary -file $OUT/ooc_v27_timing.rpt
report_drc            -file $OUT/ooc_v27_drc.rpt
report_route_status   -file $OUT/ooc_v27_route_status.rpt
write_checkpoint      -force $OUT/ooc_v27_routed.dcp
