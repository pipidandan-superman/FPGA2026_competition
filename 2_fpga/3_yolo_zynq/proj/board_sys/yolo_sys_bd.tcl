...
################################################################
# This is a generated script based on design: yolo_sys
#
...
proc create_root_design { parentCell } {
  ...
  set_property -dict [list \
    ...
    CONFIG.PCW_MIO_TREE_PERIPHERALS {GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#GPIO#SD\
0#SD 0#SD 0#SD 0#SD 0#SD 0#GPIO#GPIO#UART 1#UART 1#GPIO#GPIO#GPIO#GPIO} \
...
\
  ...
  # Create instance: engine, and set properties
  set block_name yolo_engine_top
  set block_cell_name engine
  if { [catch {set engine [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
  ...
  set_property -dict [ list \
   ...
   CONFIG.CLK_DOMAIN {yolo_sys_ps7_0_FCLK_CLK0} \
 ] [get_bd_intf_pins $engine/m_axi]
  ...
  set_property -dict [ list \
   CONFIG.FREQ_HZ {60000000} \
   CONFIG.CLK_DOMAIN {yolo_sys_ps7_0_FCLK_CLK0} \
 ] [get_bd_intf_pins $engine/x_m_axi]

  # Create instance: rst_150, and set properties
  set rst_150 [ create_bd_cell -type ip -vlnv xilinx.com:ip:proc_sys_reset:5.0 rst_150 ]

  # Create instance: lock_1, and set properties
  set lock_1 [ create_bd_cell -type ip -vlnv xilinx.com:ip:xlconstant:1.1 lock_1 ]
  ...
  connect_bd_intf_net -intf_net axi_mem_intercon_1_M00_AXI [get_bd_intf_pins axi_mem_intercon_1/M00_AXI] [get_bd_intf_pins ps7/S_AXI_HP1]
  ...
  connect_bd_intf_net -intf_net engine_m_axi [get_bd_intf_pins engine/m_axi] [get_bd_intf_pins axi_mem_intercon/S00_AXI]
  connect_bd_intf_net -intf_net engine_x_m_axi [get_bd_intf_pins engine/x_m_axi] [get_bd_intf_pins axi_mem_intercon_1/S00_AXI]
  ...
  connect_bd_intf_net -intf_net ps7_axi_periph_M00_AXI [get_bd_intf_pins ps7_axi_periph/M00_AXI] [get_bd_intf_pins engine/s_axi]

  # Create port connections
  connect_bd_net -net engine_irq_o  [get_bd_pins engine/irq_o] \
  [get_bd_pins ps7/IRQ_F2P]
  connect_bd_net -net lock_1_dout  [get_bd_pins lock_1/dout] \
  [get_bd_pins rst_150/dcm_locked]
  connect_bd_net -net ps7_FCLK_CLK0  [get_bd_pins ps7/FCLK_CLK0] \
  [get_bd_pins engine/clk_i] \
  [get_bd_pins rst_150/slowest_sync_clk] \
  ...
  [get_bd_pins rst_150/ext_reset_in]
  connect_bd_net -net rst_150_peripheral_aresetn  [get_bd_pins rst_150/peripheral_aresetn] \
  [get_bd_pins engine/rst_n] \
  ...
  assign_bd_address -offset 0x43C10000 -range 0x00010000 -target_address_space [get_bd_addr_spaces ps7/Data] [get_bd_addr_segs engine/s_axi/reg0] -force
  assign_bd_address -offset 0x00000000 -range 0x40000000 -target_address_space [get_bd_addr_spaces engine/m_axi] [get_bd_addr_segs ps7/S_AXI_HP0/HP0_DDR_LOWOCM] -force
  assign_bd_address -offset 0x00000000 -range 0x40000000 -target_address_space [get_bd_addr_spaces engine/x_m_axi] [get_bd_addr_segs ps7/S_AXI_HP1/HP1_DDR_LOWOCM] -force
...
# ============================================================
# 本地追加说明（run9c 2026-09-18，v2 导出）：
# 本文件 = write_bd_tcl 从 GUI 权威 BD 导出的正典重建脚本。
#   生成命令（GUI 关闭后批处理，fix_bd.tcl 同款）：
#     open_project ./yolo_a2_board/yolo_a2_board.xpr
#     open_bd_design [get_files yolo_sys.bd]
#     write_bd_tcl -force E:/.../yolo_sys_bd.tcl
# 由 build_board.tcl 在 create_project + add_files(RTL/XDC) 之后
# source 本文件（自带 validate_bd_design + save_bd_design）。
# 相对手工拼装（ps7_ees331.tcl 两遍 foreach）的关键优势：
#   1) ps7 全部 533+ 配置一次性 set_property -dict 灌入（大括号
#      保护 MIO_TREE 值不被二次求值），PCW_IRQ_F2P_INTR=1 在同批
#      激活 → IRQ_F2P 引脚物化（九跑实证：事后单独 set 该参数
#      得 [BD 41-721] disabled parameter ignored，引脚永不出现，
#      connect_bd_net 空引脚报 [BD 41-701]）。
#   2) 用户 GUI 手工三处（DDR/FIXED_IO make external、IRQ_F2P
#      勾选直连 engine/irq_o）+ FREQ_HZ×3/CLK_DOMAIN×3 修正全部
#      固化在此脚本。
# v2 变更：用户 GUI 保存曾把 engine/s_axi 的 FREQ_HZ 重置回默认
#   100M（m_axi/x_m_axi 存活）→ 重建时 41-237 vs auto_pc 60M；
#   已在权威 BD 三连 re-apply 后重新导出（s_axi 现带 FREQ_HZ）。
#   教训：module_ref 接口的 FREQ_HZ/CLK_DOMAIN 在 GUI 任何改动
#   保存后都可能丢——每次 GUI 改完 BD，重导出前先三连 re-apply。
# BD 若再在 GUI 中改动：先 re-apply FREQ_HZ×3，再 write_bd_tcl
#   覆盖本文件。
# ============================================================
